      ...
      <DialogContent className="outline-hidden!">
        ...
        {result && (
          <div className="my-4 flex w-full min-w-0">
            <DataField title={t('key')} desc={t('note')} icon={LockIcon} copiable value={result.token} />
          </div>
        )}

        {result?.key?.expires_at && (
          <span className="text-sm text-secondary">{t('valid', { expiry: result.key.expires_at })}</span>
        )}
