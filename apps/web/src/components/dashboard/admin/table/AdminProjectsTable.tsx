...
export default function AdminProjectsTable({ data, page }: { data: PaginatedData<AdminProjectInfo>; page: number }) {
  const columns: TableColumn<AdminProjectInfo>[] = [
    ...
    },
    {
      id: 'modid',
      header: 'Mod ID',
      cell: (item) => <span className="font-mono">{item.mod_id || '-'}</span>
    },
    {
      id: 'created_at',
      header: 'Created at',
      cell: (item) => <LocalDateTime form="LLL d, yyyy" dateTime={new Date(item.created_at)} />
    }
  ];

  return <DataTable columns={columns} data={data} page={page} linker={(r) => navigation.getDevProjectLink(r.id)} />;
}
