...
function EmptyDeploymentsState() {
  return (
    ...
    >
      ...
      <span className="text-lg font-medium">No deployments found</span>

      <span>Create a new deployment to publish your project on the wiki.</span>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-3 flex-row gap-1 overflow-hidden">
        <GitCommitHorizontal width={20} height={20} className="shrink-0" />
        <div className="flex flex-row gap-2 overflow-hidden">
          {deployment.revision ? (
            <>
              <span className="font-mono text-sm font-light">{deployment.revision.hash}</span>
              <span className="overflow-hidden pr-4 text-sm font-normal text-ellipsis whitespace-nowrap">
      ...
      <div className="flex w-full flex-1 flex-row justify-between gap-1 text-secondary sm:w-fit sm:justify-start">
        ...
        <div className="flex flex-1 text-end text-sm">
          <span className="w-full">{deployment.user_id || '-'}</span>
        </div>
      </div>
