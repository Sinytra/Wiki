...
export default function ProjectIssueWidget({ issue }: { issue: ProjectIssueInfo }) {
  ...
  const issueLevels: {
    [key in ProjectIssueLevel]: { bg: string; bg_hover: string; fg: string };
  } = {
    error: {
  ...
  return (
    ...
    >
      ...
      >
        <span className={cn('flex flex-row items-center gap-2 text-sm', activeLevel.fg)}>
          <Icon className="size-4" />
          <span className="font-medium">{t(issue.type)}</span>
        </span>

        <ChevronDown
          className={`relative top-[1px] ml-1 size-4 text-secondary transition-transform duration-200 group-data-[state=open]:rotate-180`}
          aria-hidden="true"
        />
      </div>
          ...
          <div className="space-y-1">
            {issue.file && (
              <p className="text-sm text-secondary">
                {v.rich('file', {
                  file: () => <span className="font-mono text-sm text-secondary-alt">{issue.file}</span>
                })}
              </p>
            )}
            ...
            {issue.version_name && (
              <p className="text-sm text-secondary">
                {v.rich('version', {
                  version: () => <span className="font-mono text-xsm text-secondary-alt">{issue.version_name}</span>
                })}
              </p>
            )}
            ...
            <p className="text-sm text-secondary">
              {v.rich('date', {
                date: () => (
                  <LocalDateTime
                    className="text-sm text-secondary-alt"
