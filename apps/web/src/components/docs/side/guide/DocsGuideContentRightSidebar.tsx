...
export default async function DocsGuideContentRightSidebar({ frontmatter, ctx }: ContentRightSidebarProps) {
  ...
  return (
    <DocsSidebarBase type="right" title={t('title')} className={cn('right-0 shrink-0', 'w-[96vw] sm:w-64')}>
      <div className="space-y-2 sm:w-56">
        ...
        {frontmatter.title && <h1 className="text-primarys text-center text-lg font-semibold">{frontmatter.title}</h1>}
      </div>
    </DocsSidebarBase>
  );
}
