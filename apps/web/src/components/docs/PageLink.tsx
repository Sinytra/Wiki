...
type LinkProps = ComponentPropsWithoutRef<typeof LocaleLink> & {
  local?: boolean;
};
...
export default function PageLink({ href, target, local, className, children }: LinkProps) {
  ...
  return (
    <Element
      href={href}
      target={target}
      className={cn(className, `text-link decoration-1 underline-offset-4 hover:text-blue-400/80 hover:underline`)}
    >
      {children}
    </Element>
  );
