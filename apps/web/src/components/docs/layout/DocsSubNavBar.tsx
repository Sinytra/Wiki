...
function SubPage({
  ...
  return (
    <LocaleLink
      href={`/project/${params.slug}/${params.version}/${path}`}
      className={cn('w-full sm:w-fit', disabled && 'pointer-events-none')}
    >
      <Button
        variant="ghost"
        size="sm"
        className={cn('h-8! w-full rounded-sm px-2! sm:h-7! sm:w-fit', active && `bg-secondary`)}
        disabled={disabled}
      >
...
function DocsSubNavProjectTitle({
  project,
  platformProject
}: {
  project: ProjectData;
  ...
  return (
    <div className="flex flex-row items-center gap-2 overflow-hidden">
      <ImageWithFallback src={platformProject.icon_url} alt="Logo" className="h-6 rounded-sm" width={24} height={24} />
...
function MobileDocsSubNavProjectTitle({
  project,
  platformProject
}: {
  project: ProjectData;
  ...
  return (
    <div className="flex flex-col items-center gap-2 overflow-hidden">
      ...
      <blockquote className="line-clamp-2 text-sm text-secondary">{platformProject.summary}</blockquote>
    </div>
  );
}

function DocsSubNavBarLinks({ project, locale, version }: { project: ProjectData; locale: string; version: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 sm:flex-row sm:gap-2">
      ...
      {project.info.content_count > 0 && <SubPage title="Content" icon={BoxIcon} path="content" />}
...
export default function DocsSubNavBar({
  project,
  platformProject,
  locale,
  version
  ...
  return (
    <div className="border-t-tertiary-dim fixed left-0 z-40 w-full border-y border-tertiary bg-primary-alt">
      <div className="mx-auto flex justify-center">
        <div className="mx-4 flex w-full max-w-5xl flex-row items-center justify-between py-2 sm:py-1.5">
