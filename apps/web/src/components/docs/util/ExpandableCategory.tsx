    ...
    <tr>
      <td>
        <div className="mb-0! w-full border-separate [&_td]:border-none">
          ...
          >
            <div className="mx-auto flex w-full justify-center text-center font-medium">{name}</div>
            <div className="w-8">
              <ToggleChevron className="ml-auto h-4 w-4" active={open} />
            </div>
          </div>

          {open && <div className={cn(className, 'mt-2 flex w-full flex-col rounded-md')}>{children}</div>}
        </div>
      </td>
    </tr>
  );
