    ...
    <table className={cn('[&_td]:bg-primary-alt/50', embedded && 'mb-0')}>
      ...
      <tbody>
        {rows.map((row, rowIndex) => (
          <tr key={rowIndex} className="block border-0 sm:table-row">
            {columns.map((col) => (
              <td
                key={col.key}
              ...
              >
                <div className="mb-1 font-semibold text-primary-alt sm:hidden">{col.label}</div>
        ...
        <tr>
          <td
            ...
            className="no-table-bs group cursor-pointer bg-table-soft!"
