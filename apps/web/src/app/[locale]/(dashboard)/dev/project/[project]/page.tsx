...
async function ProjectPlatforms({ project }: { project: DevProjectData }) {
  ...
  return (
    <div className="flex w-full max-w-lg min-w-0 flex-col gap-5">
      ...
      <div className="space-y-5">{...entries}</div>
    </div>
  );
}
    ...
    <div className="flex w-full max-w-lg min-w-0 flex-col gap-4">
      ...
      <div className="space-y-5">
        ...
        <DataField
          ...
          icon={StatusIcon}
          value={u(status)}
          className={statuses[status].text}
        ...
        <DataField
          ...
          icon={project.is_public ? GlobeIcon : LockIcon}
...
async function ProfileProject({ project }: { project: DevProjectData }) {
  ...
  return (
    <div className="flex flex-col justify-between gap-3 py-1">
      ...
      <div className="flex w-full flex-row gap-4 rounded-md border border-tertiary bg-primary-alt p-4">
        <ImageWithFallback
          className="size-16 rounded-sm sm:size-19"
        ...
        <div className="flex flex-col justify-between">
          <p className="font-medium text-primary sm:text-lg">{platformProject.name}</p>
          <p className="min-h-6 text-sm font-normal text-secondary sm:text-base">{platformProject.summary}</p>
        </div>
      </div>

      <div>
        <ProjectGitRevision loading={project.status === 'loading'} revision={project.revision} current />
      </div>
