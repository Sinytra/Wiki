    ...
    <div className="flex w-full flex-row gap-4 rounded-sm border border-tertiary bg-primary-alt p-4">
      ...
      <div className="flex w-full flex-col">
        <div className="mb-0.5 flex flex-row items-center justify-between">
          <span className="text-lg leading-5 text-primary-alt">{user.name}</span>
          {user.role === 'admin' && <AdminBadge />}
        </div>
        <div className="mb-auto text-sm text-secondary">{user.username}</div>
...
function UserSettingsCategory({ title, className, children }: { title: string; className?: string; children?: any }) {
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      <div className="text-lg text-primary-alt">{title}</div>
        ...
        <BreadcrumbList>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink asChild>
              <LocaleNavLink href="/dev">{t('breadcrumbs.home')}</LocaleNavLink>
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>{t('breadcrumbs.settings')}</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
