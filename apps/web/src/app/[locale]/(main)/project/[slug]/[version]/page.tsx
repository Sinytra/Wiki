...
function ExternalLink({
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer">
      <div
        className={cn(
          `flex max-w-48 min-w-36 cursor-pointer flex-row items-center justify-center gap-2 rounded-sm border bg-gradient-to-b p-2 hover:to-60% sm:p-3`,
          className
        )}
      >
...
function ProjectTags({ project }: { project: PlatformProject }) {
  ...
  return (
    <div className="space-x-2">
      {project.categories
        .filter((t) => ProjectCategories[t] !== undefined)
        .map((tag) => (
          <span key={tag} className="rounded-md bg-secondary px-2 py-1 text-xs text-secondary-alt">
    ...
    <div className="mx-auto mt-1 mb-5 flex w-full max-w-5xl flex-col gap-6">
              ...
              <div className="inline-block">
                {info.authors.map((a, idx) => (
                  <div key={a.name} className="inline-block">
                    ...
                    {idx != info.authors.length - 1 && <span>,&nbsp;</span>}
                  </div>
                ))}
              </div>
            )
      ...
      {platformProject.description.length > 0 && (
        <Section title={t('description.title')} icon={BookOpenIcon}>
          <DocsHomepage
            ...
            wrapper={(content) => <ExpandableDescription>{content}</ExpandableDescription>}
          />
        </Section>
      )}
      ...
      <Section title={t('navigation.title')} icon={MapIcon} className="flex flex-row flex-wrap gap-4">
        ...
        {project.info.content_count > 0 && (
          <SubpageLink
            ...
            icon={BoxIcon}
            desc={t('navigation.content.desc', {
              items: project.info.content_count
            })}
            href="content"
          />
        )}
      ...
      <Section title={t('links.title')} icon={LinkIcon} className="flex flex-row flex-wrap gap-2">
        ...
        {platformProject.source_url &&
          (platformProject.source_url.startsWith('https://github.com/') ? (
            <ExternalLink
              text="GitHub"
              icon={GitHubIcon}
        ...
        <LicenseBadge name={t('license.wiki')} icon={BookOpenTextIcon}>
          <span className="text-center text-base">
            {t.rich('license.default', {
              link: (chunks: any) => (
                <PageLink href="/about/tos#copyright-policy" className="!text-base" target="_blank">
                  {chunks}
                </PageLink>
              ),
              license: () => (
                <PageLink className="!text-base" href={DEFAULT_WIKI_LICENSE.url} target="_blank">
