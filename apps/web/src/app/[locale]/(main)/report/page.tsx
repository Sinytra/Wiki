  ...
  return (
    <div className="page-wrapper-ext flex w-full flex-row justify-center gap-4">
      <div className="flex w-full max-w-[62rem] flex-col gap-4">
        <h1 className="border-b border-secondary pb-4 text-center text-2xl text-primary">{t('title')}</h1>
        ...
        <div className="rounded-md bg-primary-alt p-4">
          <ClientLocaleProvider keys={['Report', 'ProjectReportReason', 'ProjectReportType']}>
            <ProjectReportForm
              ...
              type={type}
              path={path}
              locale={locales.actualLocale(locale)}
              version={commonNetwork.actualVersion(version)}
        ...
        <div>
          <span className="text-sm text-secondary">
            ...
            {t.rich('privacy', {
              link: (chunks: any) => (
                <LocaleNavLink className="font-medium underline" href="/about/privacy">
                  {chunks}
                </LocaleNavLink>
              )
            })}
