...
async function getFont() {
  ...
  return Promise.all(
    Object.entries(fonts).map(async (e) => {
...
async function projectPageImage(coords: DocsPathCoords, project: PlatformProject, fonts: any) {
  ...
  const actualUrl = project.icon_url.endsWith('.webp')
    ? (await getImageBase64(project.icon_url)).url
    : project.icon_url;
...
function docsEntryPageResponse(
  coords: DocsPathCoords,
  project: string,
  title: string,
  iconUrl: AssetLocation | null,
  fonts: any
) {
  return new ImageResponse(
    <div
