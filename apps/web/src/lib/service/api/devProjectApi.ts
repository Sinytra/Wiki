...
async function updateProject(projectId: string, body: z.infer<typeof projectUpdateSchema>): Promise<ApiCallResult> {
  return network.resolveApiCall(() =>
    network.sendDataRequest(`dev/projects/${projectId}`, {
      method: 'PUT',
      body
    })
  );
...
): Promise<ApiCallResult<PaginatedData<DeploymentInfo>>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${projectId}/deployments`, {
      parameters
    })
  );
}
...
async function getDeployment(projectId: string, deploymentId: string): Promise<ApiCallResult<DeploymentInfo>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${projectId}/deployments/${deploymentId}`)
  );
}

async function deployProject(id: string, token: string | null = null): Promise<ApiCallResult> {
  return await network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${id}/deploy`, {
      method: 'POST',
      parameters: { token }
    })
  );
...
async function removeProjectFlag(projectId: string, flag: ProjectFlag): Promise<ApiCallResult> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${projectId}/flags/${flag}`, {
      method: 'DELETE'
    })
  );
}
...
async function addProjectMember(
  projectId: string,
  body: z.infer<typeof addProjectMemberSchema>
...
async function removeProjectMember(
  projectId: string,
  body: z.infer<typeof removeProjectMemberSchema>
): Promise<ApiCallResult> {
  return network.resolveApiCall(() =>
    network.sendDataRequest(`dev/projects/${projectId}/members`, {
      method: 'DELETE',
      body
    })
  );
...
async function getProjectVersions(
  projectId: string,
  parameters: ApiRouteParameters
): Promise<ApiCallResult<DevProjectVersions>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${projectId}/versions`, {
      parameters
    })
  );
}

async function getProjectContentPages(
  id: string,
  parameters: ApiRouteParameters
): Promise<ApiCallResult<ProjectContentPages>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${id}/content/pages`, {
      parameters
    })
  );
}

async function getProjectContentTags(
  id: string,
  parameters: ApiRouteParameters
): Promise<ApiCallResult<ProjectContentTags>> {
  return network.resolveApiCall(() => network.sendSimpleRequest(`dev/projects/${id}/content/tags`, { parameters }));
}

async function getProjectContentTagItems(
  id: string,
  tag: string,
  parameters: ApiRouteParameters
): Promise<ApiCallResult<ProjectContentPages>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${id}/content/tags/${tag}`, {
      parameters
    })
  );
}

async function getProjectContentRecipes(
  id: string,
  parameters: ApiRouteParameters
): Promise<ApiCallResult<ProjectContentRecipes>> {
  return network.resolveApiCall(() =>
    network.sendSimpleRequest(`dev/projects/${id}/content/recipes`, {
      parameters
    })
  );
}
