...
async function deployProject(id: string, token: string | null = null): Promise<ApiCallResult> {
  return await network.resolveApiCall(() => network.sendSimpleRequest(`dev/projects/${id}/deploy`, {
    method: 'POST',
    parameters: {token}
  }));
}
...
async function removeProjectMember(projectId: string, body: z.infer<typeof removeProjectMemberSchema>): Promise<ApiCallResult> {
  return network.resolveApiCall(() => network.sendDataRequest(`dev/projects/${projectId}/members`, {
    method: 'DELETE',
    body
  }));
}
