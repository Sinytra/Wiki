...
{
    // * use latest directory layout, always '1'
    "schema": "1",
    // * unique project id
    "id": "examplemod",
    ...
    // in-game modid, enables content integration
    "modid": "examplemod",
    // users authorized to register this project
    "owners": [ "Su5eD" ]
}
```
...
### `schema` <span className="text-rose-400!">*</span>
  - Type: `string`
  - Speficies the metadata and directory structure version
  - Always `"1"`
...
### `platforms` <span className="text-rose-400!">*</span>
  ...
  - Map of your project's distribution platforms to project [slugs](#slug) or IDs
  - Currently supported platforms are `modrinth` and `curseforge`
  - ⚠️ If your CurseForge project only has alpha files, you MUST use the project ID instead of its slug, otherwise we
    can't find it!
...
### `versions`
  - Type: `map<string, string>`
  - A map of additional versions to git branch names to make available.
  - The default branch is implicitly included
  - See the [Versioning](#versioning) section for details
...
<Callout type="info">
    If your project does not include a license (e.g. it is "All Rights Reserved"), leave the project
    license field empty
</Callout>
