...
<Callout type="warning">
    To use this feature, you must specify the `modid` property in your [wiki metadata](wiki_meta#Format) file.
</Callout>
...
The Wiki provides vanilla and mod loader game content by default, and therefore you don't need to include them
yourself.
...
<Callout type="important">
  Make sure to include all tags provided your mod, including those under the `c` namespace. This is important for
  displaying ingredients in recipes.
</Callout>
...
To display recipes (including custom ones) in content pages, you can use the
[`PrefabObtaining`](/docs/components/interactive#prefabobtaining) component to automatically show a recipe matching the
page's ID (recommended). Alternatively, the [`ProjectRecipe`](/docs/components/common#projectrecipe) component lets you
display a recipe for a given ID.
...
<!-- Will display recipe that outputs "examplemod:steel_ingot" -->
<PrefabObtaining/>

<!-- Will display recipe with the given ID -->
...
- `inputSlots` and `outputSlots`
  ...
  - **Each slot is 32 pixels in size** (as opposed to 16 pixels in-game!). Items displayed in slots will use this size.
...
Workbenches are defined in a `workbenches.json` file inside the `data/<modid>` directory in your project.
