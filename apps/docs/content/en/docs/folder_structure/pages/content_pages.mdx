...
**in-game content**, such as *items, block, entities and others*.
...
Below you can find a content page example from [Modular Force Field Systems](https://modrinth.com/mod/mffs)
...
Content pages are traditionally focused on describing a specific object rather than providing broader tutorials.
These descriptions often include technical information, such as IDs, stats and relations.
...
[automate](../../tools/gradle.mdx) copying/generating data files in the project.
...
- [x] Easily [link](../markdown#content-page-links) to other pages
...
By default, for single-item pages, the ID doubles as the location of the page icon,
which is sourced from [assets](../assets).
...
There's several optional attributes you can specify to fully customize your page.
However, in most cases, the wiki will fill in the best defaults available.

<details>
  <summary>View extended metdata example</summary>
  ...
  # Both keys and values may contain markdown. This is useful when adding links or
  # hint text to custom properties.
  # (Optional)
  custom:
    Subcategory: Wiring
    '[Level](levels_page)': 'Advanced'
...
from the [Item Properties](../game_data) file. Additional properties can be specified in the frontmatter - see
`custom` attribute under [Extended metadata](#extended-metadata).
