...
<Steps>
    ...
    We recommend using `wiki` or `docs`.
    ...
    - `platforms` <span className="text-rose-400!">*</span>
      - Map of your project's distribution platforms to project slugs or IDs
      - Currently supported platforms are `modrinth` and `curseforge`
      - ⚠️ If your CurseForge project only has alpha files, you MUST use the project ID instead of its
        slug, otherwise we can't find it!
    - `modid` (optional)
      - In-game mod id, used to identify mod content data
      - Multiple wiki projects may share the same value
      - Required to make use of [game data](folder_structure/game_data)
    ...
    - Learn about the wiki page [format](folder_structure/markdown) you'll use to write documentation
