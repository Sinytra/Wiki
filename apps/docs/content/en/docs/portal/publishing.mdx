...
You can access our [developer portal](https://moddedmc.wiki/dev) by clicking `Log in` on the Wiki's top navigation bar.
...
to match the repository url. See [Ownership verification](#ownership-verification) for details.

### Platform project not found

- **Possible causes:**
  - The specified platform project slug is incorrect
  - If your project is hosted on CurseForge and only contains alpha files, it will not be indexed by their API

- **Fixes:**
  - Make sure the specified slugs / IDs are correct
  - In the latter case, you must use the CurseForge project's ID in the `platforms` metadata field instead of the slug

### User is not authorized to register this project

- **Possible cause:** The username of the Wiki account trying to register this project does not match any usernames in
the project metadata's [`owners` field](http://localhost:3000/docs/folder_structure/wiki_meta#format).

- **Fix:** Make sure you are logged in using the correct account, or include your current username in `owners`.
