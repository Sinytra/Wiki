...
export default function DataTable<T>({
  ...
  const headers = actualHeaders.map((col) => (
    <TableHead
      className={cn(
        'scrollbar-none overflow-auto border-x-0 border-t-0 first:border-l-0 last:border-r-0',
        col.className
      )}
      key={col.id}
    >
  ...
  const rows = isEmpty
    ...
    : data.data.map((item, i) => {
        const row = (
          <>
            {columns.map((col) => (
              <TableCell className={cn('border-x-0 first:border-l-0 last:border-r-0', col.className)} key={col.id}>
                <div className="scrollbar-none overflow-auto">{col.cell(item, i + offset)}</div>
              </TableCell>
            ))}
          </>
        );
