...
// @ts-ignore
export default function DocsSidebarBase({title, className, innerClassName, tagName, children, context = LeftSidebarContext}: DocsSidebarBaseProps) {
  const ContentDiv = tagName || 'div' as any;

  const {open} = useContext(context)!;

  return (
    <aside data-open={open} className={cn(className, 'sm:h-[calc(100vh_-_9.5rem)] transition-all lg:transition-none duration-300 ease-in-out overflow-hidden border-border bg-background')}>
