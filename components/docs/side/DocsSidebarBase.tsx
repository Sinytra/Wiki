...
const DocsSidebarBase = forwardRef<HTMLElement, DocsSidebarBaseProps>(function DocsSidebarBase({title, className, innerClassName, tagName, children, type}: DocsSidebarBaseProps, ref) {
  ...
  return (
    <aside data-open={open == type} className={cn(className, 'w-full z-30 fixed lg:sticky lg:top-[6rem]! h-[88vh] sm:h-[calc(100vh_-_9.5rem)] transition-all lg:transition-none duration-300 ease-in-out overflow-hidden border-border bg-primary')}>
