...
export default function DocsVersionSelector({version, versions}: { version: string; versions: ProjectVersions }) {
  ...
  return (
    <DocsVersionSelectWrapper value={version} defaultValue={DEFAULT_DOCS_VERSION}>
      <SelectTrigger className="sm:w-fit sm:min-w-24 sm:max-w-32 h-7 py-0 rounded-sm [&>span]:text-sm bg-transparent hover:bg-secondary
                                border-none justify-start [&>svg:last-child]:hidden [&>span]:text-ellipsis [&>span]:block
                                whitespace-nowrap">
