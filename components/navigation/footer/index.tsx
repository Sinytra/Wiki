...
function LinkEntry({title, href}: { title: string, href: string }) {
  return <>
    <ul>
      <li>
        <Link className="hover:text-secondary-foreground transition-colors rounded-md py-0.5 text-sm text-muted-foreground outline-none"
              href={href}>
          {title}
        </Link>
      </li>
...
export default function Footer() {
  return (
    <footer className="w-full bg-muted pt-12 pb-6 mx-auto flex flex-col justify-center items-center border-t border-border">
      <div className="px-8 w-full max-w-[90rem] text-muted-foreground flex flex-col gap-8">
        <div
          className="flex flex-wrap-reverse sm:flex-nowrap sm:flex-row w-full justify-center gap-y-8 sm:justify-between">
          <Copyright/>
